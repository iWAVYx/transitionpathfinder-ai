...
test("production migration baseline tooling is read-only and fail-closed", () => {
  ...
  assert.equal(preWindowReport.canonicalCount, 197);
  ...
  assert.equal(preWindowReport.pendingCount, 9);
  ...
  assert.equal(postWindowReport.canonicalCount, 197);
  ...
  assert.equal(postWindowReport.pendingCount, 6);
  ...
  assert.equal(currentProductionReport.canonicalCount, 197);
  ...
  assert.equal(currentProductionReport.pendingCount, 3);
...
test("hosted builds stay within Lovable memory limits without duplicate PWA work", () => {
  ...
  assert.match(viteConfig, /function directDateFnsModulesPlugin\(\): Plugin/);
