...
test("hosted builds stay within Lovable memory limits without duplicate PWA work", () => {
  ...
  assert.match(
    packageJson,
    /--max-semi-space-size=4 --max-old-space-size=1280 --expose-gc' vite build && node scripts\/generate-service-worker\.mjs/,
  );
  assert.match(
    packageJson,
    /--max-semi-space-size=4 --max-old-space-size=1280 --expose-gc' vite build --mode development && node scripts\/generate-service-worker\.mjs/,
  );
