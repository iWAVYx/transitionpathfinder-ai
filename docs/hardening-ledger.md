...
- **New module** `src/lib/pathway-registry-loader.server.ts` — server-only adapter (`.server.ts` suffix keeps it out of the client bundle). Reads the D1 registries via an *injected* client and feeds them into the D4 pure resolver:
  - `loadPathwayEngineInvocation({ client, promptVersion, modelVersion, channel? = "shadow", maxKnowledgeRefs? })` — queries `pathway_rules_versions` scoped to `engine_channel = <channel>` + `retired_at IS NULL`, newest `effective_at` first, then all live `pathway_knowledge_sources`, and delegates to `resolvePathwayEngineInvocation` for schema-validated `RecProvenance`.
  - Structural `RegistryClient` contract (no supabase-js import) so callers can pass `supabaseAdmin` (registries are admin-only anyway) or an edge-function service-role client, and tests can pass a plain-object stub.
  - Discriminated-union return: `{ ok:true, provenance, knowledge_dropped, rulesRow, knowledgeRows }` on success; `{ ok:false, error_code: "rules_query_failed" | "knowledge_query_failed" | "no_active_rules" | "rules_retired" | "invalid_channel" | "provenance_invalid", message }` on any expected failure. Never throws on missing rows / DB errors / bad channel — bubbles only truly unexpected client exceptions.
  - Default channel is `"shadow"` so any accidental early call cannot hit production traffic.
- **Test** `tests/pathway-registry-loader.test.mjs` — 6/6 pass under `node --experimental-strip-types --test`. Covers happy path (channel + retired_at filters asserted call-by-call), `channel: "canary"` override, missing rules row → `no_active_rules`, rules DB error → `rules_query_failed`, knowledge DB error → `knowledge_query_failed`, and empty knowledge set → valid provenance with `knowledge_ref: []`.
- **Dormant**: no server function, edge function, writer, route, or UI imports this yet. The eventual engine adapter will call `loadPathwayEngineInvocation` once per report, stamp the result on `pathway_reports` via `provenanceToReportColumns` (D4), and pass the same provenance to every `RecommendationV1` (D2) it emits.
- **Non-goals**: no supabase-js import, no schema change, no `pathway.functions.ts` / `pathway-v2.ts` / `demo/pathway-engine.ts` / worker changes, no admin UI. Wiring the loader into a real engine invocation is the next slice.
- **Rollback**: delete `src/lib/pathway-registry-loader.server.ts` and `tests/pathway-registry-loader.test.mjs`. Nothing else imports either file.

Next: **Slice D6** — dormant shadow-channel engine adapter that (1) calls the D5 loader with `supabaseAdmin`, (2) runs `assessEvidenceSufficiency` per pillar, (3) emits either a `buildAssessmentRefusal` rec or a real `RecommendationV1` batch through `parseRecommendationBatchV1`, and (4) writes the provenance columns onto a new `pathway_reports` row — all behind an off-by-default flag so no user-visible behavior changes.

### Slice D6 — Pathway shadow-channel engine adapter (DORMANT)
- **New module** `src/lib/pathway-engine-shadow.ts` — pure isomorphic orchestrator (no supabase, no react, no server imports). `runPathwayEngineShadow({ enabled, reportId, age_band, pillars[], provenance, generate })` composes the D2 schema gate + refusal path with the D4/D5 provenance and a caller-injected recommendation generator:
  - `enabled: false` short-circuits to `{ ok:true, status:"disabled", reason:"flag_off" }` **without invoking the generator** — asserted in test.
  - Per pillar: `assessEvidenceSufficiency` → if insufficient, `buildAssessmentRefusal({ id: refusalIdFor(reportId,pillar), … })`; if sufficient, call `generate()` and re-parse through `parseRecommendationBatchV1`. The generator is never trusted — its output shape is always re-validated.
  - Whole-batch re-validation with `RecommendationBatchV1` catches cross-pillar id collisions a per-pillar gate can't see.
  - Structured refusals: `duplicate_pillar` (rejected up front, no silent merge), `generator_threw` (turns caller exceptions into structured results), `schema_invalid` (per-pillar), `batch_invalid` (cross-pillar uniqueness).
  - Deterministic `refusalIdFor(reportId, pillar)` keeps refusal recs stable across regenerations — clean downstream diffs.
- **Test** `tests/pathway-engine-shadow.test.mjs` — 8/8 pass under `node --experimental-strip-types --test`: flag-off skips generator, insufficient evidence emits a schema-valid refusal, sufficient evidence delegates + re-validates, schema-invalid output short-circuits with `error_code:"schema_invalid"`, thrown generator surfaces as `generator_threw`, duplicate pillar input rejected, cross-pillar id collision fails batch uniqueness, and mixed pillars (one refused + one generated) both land in the batch with correct `per_pillar` outcomes.
- **Dormant**: no route, server function, edge function, writer, or UI imports this yet. The adapter is off by default via the `enabled` flag — even after wiring, callers must opt in. No AI call is made here; the generator is injected so the eventual server function decides between the Lovable AI Gateway and the demo engine.
- **Non-goals**: no `pathway_reports` write yet (deferred to Slice D7 — that slice will call `provenanceToReportColumns` from D4 and persist the batch with `supabaseAdmin`), no schema change, no `pathway.functions.ts` / `pathway-v2.ts` / `demo/pathway-engine.ts` / worker changes, no admin UI.
- **Rollback**: delete `src/lib/pathway-engine-shadow.ts` and `tests/pathway-engine-shadow.test.mjs`. Nothing else imports either file.

Next: **Slice D7** — dormant `pathway_reports` writer that (1) loads provenance via D5 with `supabaseAdmin`, (2) runs the D6 adapter with a real (or demo) generator, (3) stamps `rules_version | prompt_version | model_version | engine_channel | knowledge_snapshot` on a new `pathway_reports` row using `provenanceToReportColumns`, and (4) is gated by an off-by-default `pathway_engine_shadow_enabled` flag so shadow-mode runs never touch production traffic.

### Slice D7 — Pathway report writer (server-only, DORMANT)
- **Module** `src/lib/pathway-report-writer.server.ts` — ties Slice D5 (registry loader) + Slice D6 (shadow-channel engine adapter) + Slice D4's `provenanceToReportColumns` into `writePathwayReport({ enabled, reportId, age_band, pillars, promptVersion, modelVersion, channel?, maxKnowledgeRefs?, registryClient, writerClient, generate })`.
- **Flag-off is the default**: `enabled: false` returns `{ ok: true, status: "disabled", reason: "flag_off" }` and performs zero DB work — the loader is never called, the generator is never called, and the writer is never called. Shipping this file cannot mutate any `pathway_reports` row until an explicit rollout flips the flag.
- **When enabled**: loader → engine → schema-validated batch → single `updateReport(reportId, { rules_version, prompt_version, model_version, engine_channel, knowledge_snapshot, recommendations })` call. All expected failures surface as a discriminated union with a `WriteErrorCode` (`rules_query_failed | knowledge_query_failed | no_active_rules | rules_retired | invalid_channel | provenance_invalid | duplicate_pillar | generator_threw | schema_invalid | batch_invalid | write_failed`) — never throws on expected failures.
- **Injected clients**: both the D5 registry client and the writer client are structural interfaces (not supabase-js imports) so tests use plain-object stubs and the eventual server function decides between `supabaseAdmin` (registries are admin-only anyway) and a service-role client.
- **Tests** `tests/pathway-report-writer.test.mjs` — 6/6 pass under `node --experimental-strip-types --test`. Covers flag-off no-op (loader/generator/writer untouched), happy path column stamping, loader failure short-circuit, engine schema-invalid short-circuit, writer error → `write_failed`, and refusal batch still written when evidence is insufficient.
- **Non-goals**: no server function, no route, no UI, no migration. `pathway_reports.recommendations` column is assumed additive from the D1/legacy schema — the writer only sends columns the caller already permits.
- **Rollback**: delete `src/lib/pathway-report-writer.server.ts` and `tests/pathway-report-writer.test.mjs`. Nothing else imports either file.

Next: **Slice D8** — dormant TanStack `createServerFn` (`generatePathwayReportShadow`) that wires `supabaseAdmin` into the D7 writer + a Lovable-AI-Gateway generator behind an `enabled: false` flag, so an operator flip enables a shadow-channel write end-to-end without touching production reports.

### Slice D9 — Pathway report dry-run preview (server-only, DORMANT)
- **New helper** `previewPathwayReport` in `src/lib/pathway-report-writer.server.ts` — read-only cousin of D7's `writePathwayReport`. Runs the D5 loader + D6 engine adapter and returns the batch + the exact provenance columns the writer *would* stamp (`rules_version`, `prompt_version`, `model_version`, `engine_channel`, `knowledge_snapshot`, `recommendations`) — but never calls the writer. `enabled:false` short-circuits with zero DB read and zero generator call, matching D7's off-by-default guarantee.
- **New server function** `src/lib/pathway-preview-shadow.functions.ts` — dormant TanStack `createServerFn` (`previewPathwayReportShadow`). Off-by-default. When flipped: `requireSupabaseAuth` → user-scoped `has_role('admin')` check → lazy import of `supabaseAdmin` → structural loader-only registry client → `previewPathwayReport`. No writer client is built here; the preview surface is structurally incapable of mutating `pathway_reports`.
- **Tests** `tests/pathway-report-preview.test.mjs` — 5/5 pass under `node --experimental-strip-types --test`. Covers flag-off (no reads, no generator), happy-path column shape, loader failure short-circuit, `schema_invalid` on bad rec (no partial preview), and refusal-batch preview when evidence is insufficient. D7 writer regression (`tests/pathway-report-writer.test.mjs`) still 6/6 green.
- **Dormant**: no route, page, or hook imports either file. Purely an operator inspection surface for admins to diff the shadow engine's would-be output against the current report before flipping the D7 write flag.
- **Non-goals**: no schema/migration changes, no writer changes, no UI. `pathway_reports` remains untouched by this slice by construction.
