...
| ---- | ------------------------------------------------------------------ | ----- | --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------ |
| B-01 | Malware / AV scanning on document and Channel uploads              | C     | P0  | Engineering + Founder + Vendor | OPSWAT is configured in isolated staging; a clean file and the harmless EICAR test file prove fail-closed scanning, infected-object deletion, and clean-only download across both `student-documents` and `channel-attachments`; production configuration and subprocessor approval are recorded. |
