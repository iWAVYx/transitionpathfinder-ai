...
| Cloudmersive Virus Scan API (selected; not yet live) | Malware / AV scanning for document and Channel uploads | Uploaded file bytes; sanitized threat names; app-generated scan correlation ID | Server-only `CLOUDMERSIVE_API_KEY`; `src/lib/document-av-scan.server.ts` | Advanced-scan adapter is fail-closed. Free evaluation is limited to 3.5 MB per file; live staging proof, plan/region selection, DPA/subprocessor approval, and production approval remain blockers for Level C. See `blockers.md` B-01. |
