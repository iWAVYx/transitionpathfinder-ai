...
| OPSWAT MetaDefender Cloud         | Malware / AV scanning for document and Channel uploads | Uploaded file bytes and provider scan identifiers | Server-only `OPSWAT_API_KEY`; `src/lib/document-av-scan.server.ts` | Pipeline is fail-closed in code. Live staging clean/infected proof and production subprocessor approval remain blockers for Level C. See `blockers.md` B-01. |
