...
| I-04  | Role revocation propagates immediately                                      | B/C   | MANUAL REVIEW | P0 | `user_roles` UPDATE is not permitted (memory rule); revocation is DELETE → RLS denies next read. No cached client-side role escalation path found. | None | Slice 2: add explicit revocation test | Engineering | Slice 3 |
