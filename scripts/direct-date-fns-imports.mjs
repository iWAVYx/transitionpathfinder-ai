const DATE_FNS_IMPORT_PATTERN = /import\s+(type\s+)?\{([^}]*)\}\s+from\s+(["'])date-fns\3\s*;?/g;
const DATE_FNS_EXPORT_PATTERN = /export \* from ["']\.\/([A-Za-z0-9]+\.js)["'];/g;
const SPECIFIER_PATTERN = /^([A-Za-z_$][\w$]*)(?:\s+as\s+([A-Za-z_$][\w$]*))?$/;
...
export function mapDateFnsModules(indexSource) {
  ...
  for (const match of indexSource.matchAll(DATE_FNS_EXPORT_PATTERN)) {
    ...
    if (existingModule && existingModule !== moduleFile) {
      throw new Error(`Ambiguous date-fns export ${exportName}: ${existingModule}, ${moduleFile}`);
    }
