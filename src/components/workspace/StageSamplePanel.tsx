 ...
 * Transition Workspace stage. Uses the workspace typography, spacing,
 * and border system so the sample belongs to the stage flow rather
 * than floating as a disconnected mockup.
 *
 * Titles use Title Case; descriptions stay sentence case, per project
 * style rules.
 */
export interface StageSamplePanelProps {
  stage: WorkspaceStage;
  /** Where "Open the interactive sample screen" navigates to. */
  fullSampleHref: string;
  /** CTA label — kept concise, Title Case. */
...
export function StageSamplePanel({
  ...
  return (
    ...
    >
      {/* Connector back to the stage spine — visually threads the sample
          into the stage flow rather than letting it float. */}
      <span
        aria-hidden
