import { FileSearch, Sparkles, ArrowRight } from "lucide-react";
import { Link } from "@tanstack/react-router";
import { toTitleCase } from "@/lib/title-case";

/**
 * DistrictEvidenceCoverageCard — how much required transition evidence
 * exists across every building. Aggregated version of the school-level
 * TransitionEvidenceCard.
 */

export interface DistrictEvidenceRow {
  category: string;
  covered: number;
  total: number;
  laggingBuilding?: string;
  ownerHint?: string;
}

export interface DistrictEvidenceCoverageData {
  rows?: DistrictEvidenceRow[];
  documentsHref?: string;
}

const SAMPLE: Required<DistrictEvidenceCoverageData> = {
  documentsHref: "/documents",
  rows: [
    { category: "Interest Inventories on File", covered: 402, total: 452, laggingBuilding: "East Valley High", ownerHint: "School Counseling" },
    { category: "Current Vocational Evaluations", covered: 318, total: 452, laggingBuilding: "Riverbend High", ownerHint: "Transition Specialists" },
    { category: "Signed Family Priorities Intakes", covered: 351, total: 452, laggingBuilding: "East Valley High", ownerHint: "Case Managers" },
    { category: "Work-Based Learning Logs (Term)", covered: 238, total: 310, laggingBuilding: "North Ridge High", ownerHint: "Job Coaches" },
    { category: "Student Voice Statements", covered: 429, total: 452, laggingBuilding: "Riverbend High", ownerHint: "Case Managers" },
    { category: "Adult-Services Referrals (Grade 12)", covered: 78, total: 118, laggingBuilding: "East Valley High", ownerHint: "Transition Specialists" },
  ],
};

function pct(r: DistrictEvidenceRow) {
  return r.total > 0 ? Math.round((r.covered / r.total) * 100) : 0;
}
function tone(p: number) {
  if (p >= 90) return { bar: "bg-emerald-500", chip: "bg-emerald-500/10 text-emerald-700 ring-emerald-500/20 dark:text-emerald-300" };
  if (p >= 75) return { bar: "bg-sky-500", chip: "bg-sky-500/10 text-sky-700 ring-sky-500/20 dark:text-sky-300" };
  if (p >= 60) return { bar: "bg-amber-500", chip: "bg-amber-500/10 text-amber-700 ring-amber-500/20 dark:text-amber-300" };
  return { bar: "bg-rose-500", chip: "bg-rose-500/10 text-rose-700 ring-rose-500/20 dark:text-rose-300" };
}

export function DistrictEvidenceCoverageCard({
  data,
  isSample = true,
}: {
  data?: DistrictEvidenceCoverageData;
  isSample?: boolean;
}) {
  const d: Required<DistrictEvidenceCoverageData> = { ...SAMPLE, ...(data ?? {}), rows: data?.rows ?? SAMPLE.rows };

  return (
    <section
      aria-labelledby="district-evidence-title"
      className="mt-6 rounded-3xl border bg-card p-6 shadow-soft sm:p-8"
      data-testid="district-evidence-card"
    >
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 pb-4">
        <div className="min-w-0">
          <p className="tf-eyebrow flex items-center gap-1">
            <FileSearch className="h-3 w-3" aria-hidden /> District Evidence Coverage
          </p>
          <h2 id="district-evidence-title" className="mt-1 font-display text-2xl font-medium tracking-tight">
            {toTitleCase("What's Documented Across Every Building")}
          </h2>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            Every required transition artifact, aggregated across the district. Lagging buildings surface for targeted support.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {isSample && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-primary ring-1 ring-primary/20">
              <Sparkles className="h-3 w-3" aria-hidden /> Sample Preview
            </span>
          )}
          <Link
            to={d.documentsHref}
            className="inline-flex items-center gap-1 rounded-full border border-primary/40 bg-background px-3 py-1.5 text-xs font-semibold text-primary no-underline transition-colors hover:border-primary hover:bg-primary/10"
          >
            Open Documents <ArrowRight className="h-3.5 w-3.5" aria-hidden />
          </Link>
        </div>
      </header>

      <ul className="mt-5 space-y-3">
        {d.rows.map((r) => {
          const p = pct(r);
          const t = tone(p);
          return (
            <li key={r.category} className="rounded-2xl border bg-background p-4">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-foreground">{r.category}</p>
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    {r.ownerHint && <>Owner: {r.ownerHint} · </>}
                    Lagging: <span className="font-medium text-foreground/80">{r.laggingBuilding ?? "—"}</span>
                  </p>
                </div>
                <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ring-1 ${t.chip}`}>
                  {r.covered} / {r.total} · {p}%
                </span>
              </div>
              <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-muted">
                <div aria-hidden className={`h-full rounded-full ${t.bar}`} style={{ width: `${p}%` }} />
              </div>
            </li>
          );
        })}
      </ul>
    </section>
  );
}
