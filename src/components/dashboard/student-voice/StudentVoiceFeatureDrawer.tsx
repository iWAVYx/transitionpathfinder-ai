...
/**
 * Shared drawer for the Student Voice dashboard module. Mirrors the Parent /
 * Educator / School Admin / District Admin / Partner drawer contract:
 * loading · error · permission · empty · ready.
 */
export type StudentVoiceFeatureState =
  | "loading"
  | "error"
  | "permission"
  | "empty"
...
export function StudentVoiceFeatureDrawer({
  featureId,
  icon,
  onOpenChange,
  isSample = false,
  ...
  featureId: StudentVoiceFeatureId | null;
  icon?: LucideIcon;
  onOpenChange: (open: boolean) => void;
  isSample?: boolean;
  state?: StudentVoiceFeatureState;
  onRetry?: () => void;
}) {
  const detail = featureId ? STUDENT_VOICE_FEATURE_DETAILS[featureId] : null;
  const Icon = icon;
...
function ReadyBody({
  ...
  detail: (typeof STUDENT_VOICE_FEATURE_DETAILS)[StudentVoiceFeatureId];
}) {
  return (
    <>
