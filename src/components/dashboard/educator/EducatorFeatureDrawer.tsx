 ...
 * Parent drawer contract so QA and future maintenance stay predictable.
 * States: loading · error · permission · empty · ready.
 */
export type EducatorFeatureState = "loading" | "error" | "permission" | "empty" | "ready";

export function EducatorFeatureDrawer({
  featureId,
  icon,
  onOpenChange,
  isSample = false,
  ...
  featureId: EducatorFeatureId | null;
  icon?: LucideIcon;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Your caseload data is safe. Try again, or open the full page.
      </p>
      {onRetry && (
        <Button
          type="button"
...
function PermissionBody({ featureTitle }: { featureTitle: string }) {
  return (
    <div className="rounded-2xl border bg-muted/30 p-5 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Your role doesn't include this section yet. Ask your school admin to
        grant caseload access.
      </p>
    </div>
  );
}
...
/** Empty-state CTA text nudges the educator toward the exact next step. */
function emptyCta(id: EducatorFeatureId): string {
  switch (id) {
    case "case-notes":
      return "Add A Note";
    case "action-items":
