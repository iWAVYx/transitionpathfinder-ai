 ...
 * back to a blank body:
 *  - loading         : we're fetching data
 *  - error           : fetch failed — offer a retry
 *  - permission      : caller isn't allowed to see this feature yet
 *  - empty           : no data yet — coach the family on what to do
 *  - ready (default) : render the sample rows
 */
export type ParentFeatureState = "loading" | "error" | "permission" | "empty" | "ready";

export function ParentFeatureDrawer({
  featureId,
  icon,
  onOpenChange,
  isSample = false,
  ...
  isSample?: boolean;
  /** Which body variant to render. Defaults to "ready". */
  state?: ParentFeatureState;
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Your data is safe. Try again, or open the full page.
      </p>
      {onRetry && (
        <Button
          type="button"
...
function PermissionBody({ featureTitle }: { featureTitle: string }) {
  return (
    <div className="rounded-2xl border bg-muted/30 p-5 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Ask your student or your team to share this section with you.
        Sharing controls live under Sharing & Consent.
      </p>
    </div>
  );
}
...
/** Empty-state CTA text nudges the family toward the exact next step. */
function emptyCta(id: ParentFeatureId): string {
  switch (id) {
    case "documents":
