...
function EndToEndTour() {
  const { profile } = useDemoStudent();
  const rows = DEMO_PROFILE_ORDER.map((id) => {
    const p = DEMO_PROFILES[id];
    const matches = matchOpportunities(p);
    const visible = matches.filter((m) => m.band !== "filtered_out");
    const hidden = matches.length - visible.length;
    return { p, visible: visible.length, hidden, strong: visible.filter((m) => m.band === "strong").length };
  });

  return (
    <section className="rounded-3xl border bg-card p-6 shadow-soft sm:p-8">
      <div className="mb-4 flex items-baseline gap-3">
        <span className="font-mono text-xs font-semibold tracking-widest text-primary">2.5</span>
        <div className="h-px flex-1 bg-border" />
        <h2 className="font-display text-xl">End-To-End: Profile → Pathway → Partner Network</h2>
      </div>

      <div className="flex flex-wrap items-start justify-between gap-4">
        <p className="max-w-2xl text-sm text-muted-foreground">
          Pick a demo student. The same profile drives the age-aware Pathway Report,
          the explainable opportunity matcher, and the Partner Network catalog.
          Age-safeguards are always on — the "filtered" counts show what the engine
          hid because it doesn't belong yet for that grade band.
        </p>
        <StudentSwitcher />
      </div>

      <ul className="mt-5 grid gap-3 sm:grid-cols-3">
        {rows.map(({ p, visible, hidden, strong }) => (
          <li
            key={p.id}
            className={`rounded-2xl border p-4 ${p.id === profile.id ? "border-primary/60 bg-primary/5" : "border-border bg-background"}`}
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-foreground">{p.displayName}</p>
                <p className="text-xs text-muted-foreground">
                  {p.demographics.gradeLabel} · Age {p.demographics.age}
                </p>
              </div>
            ...
            <p className="mt-2 text-xs text-foreground/80">{p.tagline}</p>
            <p className="mt-2 text-[11px] text-muted-foreground">
              {visible} matches shown · {hidden} filtered
            </p>
          </li>
        ))}
      </ul>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        <Button asChild size="sm" variant="outline">
          <Link to="/demo/report" search={{ student: profile.id }}>
            <FileText className="mr-1.5 h-4 w-4" /> Pathway Report
          </Link>
        </Button>
        <Button asChild size="sm" variant="outline">
          <Link to="/demo/opportunities" search={{ student: profile.id }}>
            <Sparkles className="mr-1.5 h-4 w-4" /> Explainable Matches
          </Link>
        </Button>
        <Button asChild size="sm" variant="outline">
          <Link to="/demo/network" search={{ student: profile.id }}>
            <Network className="mr-1.5 h-4 w-4" /> Partner Network
          </Link>
        </Button>
      </div>

      <p className="mt-4 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-primary">
        <ShieldCheck className="h-3 w-3" />
        Age-Safeguards Active For All Three Profiles
      </p>
    </section>
  );
}
